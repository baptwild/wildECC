...
### clone du dépôt
```bash
git clone https://github.com/baptwild/wildECC
cd wildECC
```

### Installation dépendence

Selon l'installation de python en local : 
```bash
pip install cryptography
```
ou 
```bash
python3 install cryptography
```
...
```bash
python3 wildECC.py keygen
```
Fichiers générés :
monECC.pub  // monECC.priv

### Génération avec nom de fichiers personnalisé

```bash
python3 wildECC.py keygen -f alice
```
Fichiers générés :
alice.pub  // alice.priv

### Chiffrer un message
```bash
python3 wildECC.py crypt monECC.pub "Pour moi ce TP mérite une bonne note."
```
...
Exemple :
5;0|dYCCxWoSXlhPklAPYO2UVBENdeppFw2I7ABmx2RvIfcNw+rLNHhMrovGriguNQl6
...
```bash 
python3 wildECC.py decrypt monECC.priv "5;0|dYCCxWoSXlhPklAPYO2UVBENdeppFw2I7ABmx2RvIfcNw+rLNHhMrovGriguNQl6"

```

Sortie :
Pour moi ce TP mérite une bonne note.

---
## Gestion des paramètres

Le programme distingue clairement :
- les arguments positionnels (commande, clé, texte)
- les options (switchs comme `-f`)

Les options sont retirées de la ligne de commande avant l’analyse des arguments positionnels afin de garantir la compatibilité des commandes `crypt` et `decrypt`.
...
⚠️ Note : Un switch optionnel -f a été implémenté pour la commande keygen afin de permettre de choisir le nom des fichiers de clés générés.
Les autres switchs optionnels proposés dans le sujet (-i, -o, -s, etc.) n’ont volontairement pas été implémentés afin de conserver un programme lisible et centré sur les mécanismes cryptographiques.
---
...
Le schéma implémenté est un chiffrement hybride :
- l’ECC est utilisée pour établir un secret partagé
- le chiffrement effectif du message est réalisé par AES
...
Principe :
- génération d’un secret ECC éphémère (kP)
- dérivation d’une clé symétrique via SHA-256
- chiffrement du message avec AES-128-CBC
---
